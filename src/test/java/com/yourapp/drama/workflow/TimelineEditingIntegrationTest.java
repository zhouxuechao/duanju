...
class TimelineEditingIntegrationTest {
    ...
    @Test void reflowMovesOnlyAudioLinkedToTheSpecificRepeatedShotClip(){
        ObjectNode project=store.create(PROJECT,obj().put("name","重复镜头").put("idea","A-B-A"));ObjectNode episode=store.create(EPISODE,obj().put("projectId",id(project)).put("name","第一集"));ObjectNode scene=store.create(SCENE,obj().put("projectId",id(project)).put("episodeId",id(episode)).put("name","室内"));
        ObjectNode shotA=store.create(SHOT,obj().put("projectId",id(project)).put("sceneId",id(scene)).put("purpose","人物反应").put("action","抬眼").put("duration",3)),shotB=store.create(SHOT,obj().put("projectId",id(project)).put("sceneId",id(scene)).put("purpose","信息揭示").put("action","开门").put("duration",3));
        ObjectNode takeA=createTake(project,shotA,"repeat-a"),takeB=createTake(project,shotB,"repeat-b"),timeline=store.create(TIMELINE,obj().put("projectId",id(project)).put("episodeId",id(episode)).put("durationMs",9000).put("contentRevision",1));
        ObjectNode clipA1=store.create(TIMELINE_ITEM,videoItem(project,timeline,shotA,takeA,0,3000));
        ObjectNode clipB=store.create(TIMELINE_ITEM,videoItem(project,timeline,shotB,takeB,3000,3000));
        ObjectNode clipA2=store.create(TIMELINE_ITEM,videoItem(project,timeline,shotA,takeA,6000,3000));
        ObjectNode audioA1=store.create(TIMELINE_ITEM,obj().put("projectId",id(project)).put("timelineId",id(timeline)).put("shotId",id(shotA)).put("linkedVideoTimelineItemId",id(clipA1)).put("track","DIALOGUE").put("startMs",100).put("durationMs",500));
        ObjectNode audioA2=store.create(TIMELINE_ITEM,obj().put("projectId",id(project)).put("timelineId",id(timeline)).put("shotId",id(shotA)).put("linkedVideoTimelineItemId",id(clipA2)).put("track","DIALOGUE").put("startMs",6100).put("durationMs",500));

        studio.update(TIMELINE_ITEM,id(clipB),obj().put("revision",revision(clipB)).put("sourceOutMs",2500).put("durationMs",2500));

        assertThat(store.get(TIMELINE_ITEM,id(audioA1)).path("startMs").asLong()).isEqualTo(100);
