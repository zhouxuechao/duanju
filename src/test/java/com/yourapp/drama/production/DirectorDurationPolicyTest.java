...
class DirectorDurationPolicyTest {
    ...
    @Test void fastSilentReactionCanBeEditedShorterThanProviderMinimum(){
        ...
        assertThat(duration.editDuration()).isBetween(1.25,2.0);assertThat(duration.providerDuration()).isEqualTo(2.0);
    }
    @Test void dialogueAndInformationRevealReceiveEnoughTime(){
        ObjectNode shot=shot("REVEAL_INFORMATION",1);shot.withArray("dialogues").add(mapper.createObjectNode().put("displayText","你父亲根本不是意外死亡。" ).put("endMs",3200));
        DirectorDurationPolicy.DurationPlan duration=policy.recommend(shot,beat("CLIMAX"),plan("TENSION_BUILD",3));
        assertThat(duration.editDuration()).isGreaterThanOrEqualTo(3.2);assertThat(duration.providerDuration()).isBetween(duration.editDuration(),5.0);
    }
