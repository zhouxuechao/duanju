...
class VolcengineAdaptersTest {
    ...
    private ObjectNode vlmResult(){ObjectNode r=mapper.createObjectNode();for(String name:VisualQualityProtocol.METRICS)r.set(name,mapper.createObjectNode().put("score",96).put("pass",true).put("confidence",.96).put("reason","符合"));r.putArray("characters").addObject().put("characterId","actor").put("characterName","主角").set("identity",mapper.createObjectNode().put("score",97).put("pass",true).put("confidence",.98).put("reason","同一角色"));r.putArray("failureCodes");r.put("overallScore",96).put("overallConfidence",.96).put("decision","PASS").put("failureOriginHint","UNKNOWN").put("reason","全部约束符合");return r;}
