...
class SeedAudioVoiceGeneratorTest {
    ...
    @Test void sendsApprovedBase64AudioReferenceInsteadOfAProviderSpeakerId(){String reference=Base64.getEncoder().encodeToString(new byte[]{1,2,3,4});VoiceGenerator.VoiceResult result=voice.generate(new VoiceGenerator.VoiceRequest("dialogue","别开门","","普通话",1,prompt(),Map.of("referenceAudioData",reference)));assertThat(result.durationSeconds()).isEqualTo(2);assertThat(submitted.get().path("references").get(0).path("audio_data").asText()).isEqualTo(reference);assertThat(submitted.get().path("references").get(0).path("speaker").isMissingNode()).isTrue();assertThat(submitted.get().path("text_prompt").asText()).contains("@音频1");}
    @Test void rejectsTwoVoiceReferencesTogether(){assertThatThrownBy(()->voice.generate(new VoiceGenerator.VoiceRequest("dialogue","别开门","provider-voice","普通话",1,prompt(),Map.of("referenceAudioData","AQI=")))).isInstanceOfSatisfying(ProviderException.class,e->assertThat(e.code()).isEqualTo("VOICE_REFERENCE_REQUIRED"));assertThat(calls.get()).isZero();}
    private VoiceGenerator.VoiceRequest request(double speed){return new VoiceGenerator.VoiceRequest("dialogue","别开门","test-speaker","普通话",speed,prompt(),Map.of());}
    private String prompt(){return "[VOICE IDENTITY]\n说话者音色严格采用 @音频1。\n[SPEECH CONTENT]\n唯一允许说出的对白：别开门\n[OUTPUT CONSTRAINTS]\n只输出单人干声。";}
}
