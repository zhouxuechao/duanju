...
class TimelineClipAnchorMigrationTest {
    ...
    private static final String URL="jdbc:h2:mem:timeline-v25;MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE;DB_CLOSE_DELAY=-1";
    ...
    @Test void relinksUniqueLegacyClipsAndMarksAmbiguousOrMissingRows() throws Exception {
        ...
        Flyway.configure().dataSource(URL,"sa","").locations("classpath:db/migration/common").target("25").load().migrate();
        try(Connection c=DriverManager.getConnection(URL,"sa","")){
            ...
            assertThat(document(c,"missing").path("timelineRelinkReason").asText()).isEqualTo("VIDEO_CLIP_NOT_FOUND");
