...
/** Add durable director-plan and shot-detail stages to the constrained job type column. */
public class V11__staged_director_jobs extends BaseJavaMigration {
    @Override public void migrate(Context context) throws Exception {
        Connection connection=context.getConnection();List<String> constraints=new ArrayList<>();
        ...
        try(Statement statement=connection.createStatement();ResultSet rows=statement.executeQuery(query)){
            while(rows.next())if(rows.getString(2).contains("'ASSET_IMAGE'")&&rows.getString(2).contains("'SHOT_PLAN'"))constraints.add(rows.getString(1));
        }
        ...
        try(Statement statement=connection.createStatement()){
            ...
            statement.execute("ALTER TABLE \"generation_job\" ADD CONSTRAINT ck_generation_job_type CHECK (type IN ('STORY','CHARACTER_PLAN','CHARACTER_LOOK','LOCATION_LOOK','SCRIPT','SHOT_PLAN','DIRECTOR_PLAN','SHOT_DETAIL','STORYBOARD','KEYFRAME','KEYFRAME_QC','VIDEO','VIDEO_QC','REPAIR','TTS','LIPSYNC','TIMELINE','RENDER','ARCHIVE','ASSET_IMAGE'))");
        }
    }
}
