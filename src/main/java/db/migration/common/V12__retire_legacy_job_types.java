...
/** Keep legacy jobs readable while preventing obsolete types from re-entering the active queue. */
public class V12__retire_legacy_job_types extends BaseJavaMigration {
        ...
        try(Statement statement=connection.createStatement()){
            ...
            statement.execute("""
                ALTER TABLE "generation_job" ADD CONSTRAINT ck_generation_job_type CHECK (
                  type IN ('STORY','SCRIPT','DIRECTOR_PLAN','SHOT_DETAIL','STORYBOARD','KEYFRAME','KEYFRAME_QC','VIDEO','VIDEO_QC','TTS','LIPSYNC','TIMELINE','RENDER','ARCHIVE','ASSET_IMAGE')
