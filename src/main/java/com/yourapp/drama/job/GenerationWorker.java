...
public class GenerationWorker {
    ...
    private void archive(ObjectNode job){
        ...
        store.transaction(()->{ObjectNode latest=store.getForUpdate(kind,targetId),next=latest.deepCopy().put("archiveUrl",finalArchive).put("archiveStatus","SUCCEEDED");if(finalMetadata!=null){next.setAll(finalMetadata);next.put("mediaProbeStatus","SUCCEEDED");}store.update(kind,targetId,revision(latest),next);ObjectNode result=obj().put("archiveUrl",finalArchive);if(finalMetadata!=null)result.set("mediaMetadata",finalMetadata);jobs.succeed(id(job),result);if(kind==VIDEO_TAKE){String reviewKey="auto-video-qc:"+targetId;boolean allowRepair=latest.path("inputSnapshot").path("clientRequestSnapshot").path("allowAutomaticRepair").asBoolean(true);jobs.enqueue(project(latest),required(latest,"shotId"),"VIDEO_QC",obj().put("takeId",targetId).put("apply",true).put("allowAutomaticRepair",allowRepair).put("requestKey",reviewKey),reviewKey);}return null;});
    }
