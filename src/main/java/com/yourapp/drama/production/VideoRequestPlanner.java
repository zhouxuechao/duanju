...
public class VideoRequestPlanner {
    ...
    public ObjectNode plan(JsonNode prepared,JsonNode context,JsonNode keyframe,JsonNode previous,JsonNode body){
        ...
        result.set("referenceMapping",array(mapping));result.set("referenceAuthority",authority(selected));result.set("referenceBudget",budgetJson(profile,budget));result.set("preflight",preflightJson(check));ObjectNode parameters=parameters(body,audio,profile);result.set("providerParameters",parameters);
        for(String field:List.of("resolution","ratio","duration","generate_audio","watermark"))if(parameters.has(field))result.set(field,parameters.path(field).deepCopy());
        result.put("desiredDuration",parameters.path("desiredDuration").asDouble()).put("providerDuration",parameters.path("duration").asInt()).put("durationAdaptationReason",parameters.path("durationAdaptationReason").asText()).put("nativeAudio",parameters.path("generate_audio").asBoolean());
    ...
    private ObjectNode parameters(JsonNode body,AudioGenerationPolicy audio,VideoModelProfile profile){ObjectNode options=body.path("providerOptions").isObject()?((ObjectNode)body.path("providerOptions")).deepCopy():mapper.createObjectNode();if(audio!=AudioGenerationPolicy.POST_ONLY&&!profile.supportsNativeAudio())throw new IllegalArgumentException("NATIVE_AUDIO_UNSUPPORTED");String ratio=options.path("ratio").asText("");if(!ratio.isBlank()&&!profile.supportedRatios().contains(ratio))throw new IllegalArgumentException("VIDEO_RATIO_UNSUPPORTED_BY_PROFILE: "+ratio);String resolution=options.path("resolution").asText("");if(!resolution.isBlank())profile.requireResolution(resolution);double desired=options.path("duration").asDouble();int provider=profile.providerDuration(desired);options.put("desiredDuration",desired).put("duration",provider).put("durationAdaptationReason",Math.abs(desired-provider)<.0001?"EXACT":"ROUND_UP_TO_PROVIDER_RANGE");options.put("generate_audio",audio!=AudioGenerationPolicy.POST_ONLY);options.putIfAbsent("watermark",com.fasterxml.jackson.databind.node.BooleanNode.FALSE);return options;}
