...
public final class PromptBudgeter {
    ...
    public String compile(List<Section> input,int maxChars){
        ...
        while(length(selected)>maxChars){Section removable=selected.stream().filter(s->!s.required()).min(Comparator.comparingInt(Section::priority)).orElse(null);if(removable==null)throw new IllegalArgumentException("PROMPT_BUDGET_REQUIRED_SECTION_OVERFLOW：身份、动作、终点或关键连续性约束超过安全长度；requiredSections="+requiredSizes(selected));selected.remove(removable);}
    ...
    private String requiredSizes(List<Section> values){StringJoiner sizes=new StringJoiner(",");for(Section value:values)if(value.required())sizes.add(value.name()+"="+value.content().length());return sizes.toString();}
