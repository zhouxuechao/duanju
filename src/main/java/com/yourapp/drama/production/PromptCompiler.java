...
public class PromptCompiler {
    ...
    public PromptResult compileImage(JsonNode request){
        ...
        sections.add(section("CURRENT LOOK / VISIBLE STATE","本次只绘制镜头第0秒："+promptVisibleState(shot,visible)+"。年龄、脸型、五官比例、发型、伤痕、当前定妆和伪装以当前状态及批准定妆为准。当前镜头必要叙事上下文："+narrativeContext(shot,request),98,true));
        sections.add(section("LOCATION IDENTITY / LAYOUT LOCK","Location Bible: "+visualCanon(plan.inheritedConstraints().path("location"))+"\n固定空间拓扑合同："+compact(topology,List.of("locationId","relationsToPreserve","visibilityMode","visibilityRule","invariant"))+"。位于摄影机后方或视锥外的固定设施不得错误出现在当前可见承载面；不同承载面上的设施不得互换、复制或合并。浅景深或背景虚化只允许降低纹理清晰度，不得改变任何固定设施的所属承载面。门窗、开口尺寸保持不变。",98,true));
    ...
    private String imagePose(String task,Shot shot,JsonNode request){String value="镜头第0秒只呈现 startState 中的姿态、视线与接触点；performance 只决定表演起势，后续动作不得提前完成。";if("PREVIS".equals(task))value+=" 构图预演可表达当前动作意图。";return value;}
    ...
    private String constraints(Shot shot,ContinuityPlan plan,JsonNode assets){return "身份、定妆、场景布局和道具外观严格沿用上方各专用锁定段，不在此重复。Scene State="+visibleState(shot,plan)+"；Style Bible="+assets.path("style").asText("写实");}
    ...
    private ObjectNode narrativeContext(Shot shot,JsonNode request){ObjectNode result=mapper.createObjectNode(),characters=result.putObject("characterStates"),knowledge=result.putObject("characterKnowledge");for(String id:shot.characterIds()){if(request.path("characterStates").has(id))characters.set(id,compact(request.path("characterStates").path(id),List.of("state","age","lookId","physicalCondition","injuries","emotionalState","clothingState","makeupState","hairState","currentGoal")));if(request.path("characterKnowledge").has(id))knowledge.set(id,compact(request.path("characterKnowledge").path(id),List.of("knownFacts","unknownFacts","suspicions","misunderstandings")));}result.set("storyFacts",relevantStoryFacts(shot,request));ObjectNode props=result.putObject("propStates");for(String id:shot.propIds())if(request.path("propStates").has(id))props.set(id,compact(request.path("propStates").path(id),List.of("state","condition","visible","carriedBy","heldByHand")));ArrayNode relationships=result.putArray("relationships");int count=0;for(JsonNode relationship:request.path("relationships")){String from=text(relationship,"fromCharacterId"),to=text(relationship,"toCharacterId");if((shot.characterIds().contains(from)||shot.characterIds().contains(to))&&count++<12)relationships.add(compact(relationship,List.of("fromCharacterId","toCharacterId","relationshipType","state","trust","hostility")));}if(request.has("storyTime"))result.set("storyTime",request.path("storyTime").deepCopy());return result;}
    private ObjectNode promptVisibleState(Shot shot,JsonNode source){ObjectNode result=mapper.createObjectNode();for(String field:List.of("locationId","time","lighting"))if(source.has(field))result.set(field,source.path(field).deepCopy());ObjectNode characters=result.putObject("characters"),props=result.putObject("props");for(String id:shot.characterIds())if(source.path("characters").has(id))characters.set(id,compact(source.path("characters").path(id),List.of("identityId","lookId","position","lookDirection","pose","physicalCondition","injuries","emotionalState","clothingState","makeupState","hairState","holding","leftHandProp","rightHandProp","actionState")));for(String id:shot.propIds())if(source.path("props").has(id))props.set(id,compact(source.path("props").path(id),List.of("holder","carriedBy","heldByHand","position","state","condition","visible")));return result;}
    private ObjectNode compact(JsonNode source,List<String> fields){ObjectNode result=mapper.createObjectNode();for(String field:fields)if(source.has(field))result.set(field,source.path(field).deepCopy());return result;}
    ...
    private List<JsonNode> references(Shot shot,JsonNode request,ContinuityPlan plan){
        List<JsonNode> characters=new ArrayList<>(),locations=new ArrayList<>(),props=new ArrayList<>(),refs=new ArrayList<>();JsonNode assets=request.path("assets"),selections=request.path("shot").path("referenceViews");boolean masterOnly="A1".equals(text(request.path("shot").path("directorRuleProfile"),"assetDependency"));
        ...
        for(String id:shot.characterIds()){JsonNode character=findAsset(assets,"characters",id);String lookId=required(plan.startState().path("characters").path(id),"lookId");JsonNode look=findAsset(assets,"looks",lookId);ObjectNode ref=selectedReference("CHARACTER_LOOK",look,selections.path(lookId).asText(),text(character,"name")+" / "+text(look,"name"),masterOnly);ref.put("characterId",id).put("subjectId",id);characters.add(ref);}
        JsonNode location=findAsset(assets,"locations",shot.locationId());if(!masterOnly){ObjectNode locationRef=selectedReference("LOCATION",location,locationView,text(location,"name"),false);locationRef.put("subjectId",shot.locationId());locations.add(locationRef);}ObjectNode layoutRef=selectedReference("LOCATION_LAYOUT",location,"LAYOUT",text(location,"name")+" / 固定空间布局（不直接入画）",masterOnly);layoutRef.put("subjectId",shot.locationId());locations.add(layoutRef);
        for(String id:shot.propIds()){JsonNode prop=findAsset(assets,"props",id);ObjectNode ref=selectedReference("PROP",prop,selections.path(id).asText(),text(prop,"name"),masterOnly);ref.put("subjectId",id);props.add(ref);}
    ...
    private ObjectNode selectedReference(String role,JsonNode asset,String view,String name,boolean masterOnly){if(view.isBlank())throw new IllegalArgumentException("分镜没有指定素材视角："+name);JsonNode selected=null;for(JsonNode item:asset.path("approvedViews"))if(item.path("approved").asBoolean()&&!item.path("stale").asBoolean()&&(view.equals(text(item,"view"))||masterOnly)){selected=item;break;}if(selected!=null){JsonNode item=selected;List<String> controls=switch(role){case "CHARACTER_LOOK"->List.of("characterIdentity","face","apparentAge","wardrobe");case "LOCATION"->List.of("locationIdentity","materials","perspective");case "LOCATION_LAYOUT"->List.of("worldTopology","fixedFacilityPosition");default->List.of("propIdentity","propGeometry","scale");};List<String> forbidden=switch(role){case "CHARACTER_LOOK"->List.of("pose","camera","background");case "LOCATION","LOCATION_LAYOUT"->List.of("characterIdentity","characterPose");default->List.of("holderPose","background");};int priority=switch(role){case "CHARACTER_LOOK"->100;case "LOCATION"->96;case "LOCATION_LAYOUT"->94;default->98;};ObjectNode ref=referenceBinding(role,required(item,"id"),required(item,"providerUrl"),required(asset,"id"),item.path("setVersion").asInt(),name,controls,forbidden,priority);ref.put("viewId",required(item,"id")).put("view",text(item,"view")).put("setVersion",item.path("setVersion").asInt()).put("name",name);return ref;}throw new IllegalArgumentException("缺少已批准的素材视角："+name+" / "+view);}
