...
public final class ProviderCapabilityContract {
    ...
    public ObjectNode verifyCanary(ImageModelProfile imageProfile,VideoModelProfile videoProfile,JsonNode evidence){
        ...
        String quality=required(image,"requestedImageQuality"),ratio=required(image,"aspectRatioIntent"),providerSize=required(image,"providerSize"),resolution=required(video,"requestedResolution"),videoRatio=required(video,"ratio");
        ...
        boolean resolutionVerified=resolutionMatches(videoProfile,resolution,videoRatio,videoWidth,videoHeight);
    ...
    private boolean resolutionMatches(VideoModelProfile profile,String resolution,String ratio,int width,int height){return profile.outputProfiles().stream().anyMatch(output->output.matches(resolution,ratio,width,height));}
