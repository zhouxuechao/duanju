...
public final class ContinuityCompatibilityEvaluator {
    public List<ProductionModels.Risk> comparePlanned(JsonNode previousEnd, JsonNode currentStart) {
        ...
        return compareStates(previousEnd, currentStart);
    }
    ...
    public List<ProductionModels.Risk> compareObserved(JsonNode previousTake, JsonNode currentStart) {
        ...
        if (statePresent(observed) && statePresent(currentStart)) risks.addAll(compareStates(observed, currentStart));
        return List.copyOf(risks);
    }

    public List<ProductionModels.Risk> compareStates(JsonNode previousEnd, JsonNode currentStart) {
        List<ProductionModels.Risk> risks = new ArrayList<>();
        compareScalar(risks, previousEnd, currentStart, "locationId", "LOCATION_DISCONTINUITY");
        compareScalar(risks, previousEnd, currentStart, "lighting", "LIGHTING_DISCONTINUITY");
        previousEnd.path("characters").fields().forEachRemaining(entry -> {
            ...
            compareScalar(risks, entry.getValue(), next, "identityId", "IDENTITY_DISCONTINUITY", path);
            compareScalar(risks, entry.getValue(), next, "lookId", "WARDROBE_DISCONTINUITY", path);
            compareScalar(risks, entry.getValue(), next, "pose", "POSE_DISCONTINUITY", path);
            compareScalar(risks, entry.getValue(), next, "motionPhase", "MOTION_PHASE_DISCONTINUITY", path);
            compareScalar(risks, entry.getValue(), next, "screenDirection", "SCREEN_DIRECTION_DISCONTINUITY", path);
            compareActionState(risks, entry.getValue().path("actionState"), next.path("actionState"), path + ".actionState");
        });
        previousEnd.path("props").fields().forEachRemaining(entry -> {
            JsonNode next = currentStart.path("props").path(entry.getKey());
            if (!next.isMissingNode()) compareScalar(risks, entry.getValue(), next, "holder", "PROP_HOLDER_DISCONTINUITY", "props." + entry.getKey());
        });
        return List.copyOf(risks);
    }
    ...
    private void compareActionState(List<ProductionModels.Risk> risks, JsonNode previous, JsonNode current, String path) {
        boolean before = previous.isObject() && !previous.isEmpty(), after = current.isObject() && !current.isEmpty();
        if (before != after) risks.add(error("ACTION_STATE_MISSING", path, "连续动作的 ActionState 必须从上一镜结束状态传到下一镜开始状态"));
    ...
    private static void compareScalar(List<ProductionModels.Risk> risks, JsonNode left, JsonNode right, String field, String code) { compareScalar(risks, left, right, field, code, field); }
    private static void compareScalar(List<ProductionModels.Risk> risks, JsonNode left, JsonNode right, String field, String code, String path) {
        String before = left.path(field).asText(""), after = right.path(field).asText("");
        if (!before.isBlank() && !after.isBlank() && !before.equals(after)) risks.add(error(code, path + "." + field, "相邻镜头状态不连续：" + before + " -> " + after));
    }
