...
public final class MockVoiceGenerator implements VoiceGenerator {
    @Override public VoiceResult generate(VoiceRequest request){
        if(request.speechText()==null||request.speechText().isBlank())throw new IllegalArgumentException("speechText 不能为空");
        double duration=Math.max(1,Math.min(10,request.speechText().codePointCount(0,request.speechText().length())*.24));
