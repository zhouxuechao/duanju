...
public final class SeedAudioVoiceGenerator implements VoiceGenerator {
    ...
    @Override public VoiceResult generate(VoiceRequest request){
        String speech=Objects.toString(request.speechText(),"").replaceAll("\\s+"," ").trim();if(speech.isBlank())throw ProviderException.invalid("SPEECH_TEXT_REQUIRED","TTS 只接受非空 speechText");if(speech.length()>1200)throw ProviderException.invalid("SPEECH_TEXT_TOO_LONG","单条对白不能超过 1200 字");
