...
public class StoryQualityService {
    ...
    private ObjectNode input(ObjectNode document){ObjectNode input=Documents.obj().put("pipelineVersion",2).put("phase","STORY_QA").put("documentId",Documents.id(document)).put("episodeNo",document.path("episodeNo").asInt());
        ...
        ObjectNode evidenceWorld=input.putObject("evidenceWorld");ArrayNode characterIds=evidenceWorld.putArray("characterIds"),locationIds=evidenceWorld.putArray("locationIds");core.path("content").path("characters").forEach(value->characterIds.add(Documents.text(value,"characterKey")));core.path("content").path("locations").forEach(value->locationIds.add(Documents.text(value,"locationKey")));
