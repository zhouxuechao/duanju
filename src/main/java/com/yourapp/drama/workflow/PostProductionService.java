...
public class PostProductionService {
    ...
    private final StoryDevelopmentService development;private final WorkflowService workflow; private final VoiceStateResolver voiceStates;private final TimelineQualityService timelineQuality;
    public PostProductionService(DocumentStore store,JobService jobs,ProductionService production,StoryDevelopmentService development,WorkflowService workflow,VoiceStateResolver voiceStates,TimelineQualityService timelineQuality){this.store=store;this.jobs=jobs;this.production=production;this.development=development;this.workflow=workflow;this.voiceStates=voiceStates;this.timelineQuality=timelineQuality;}
    ...
    public ObjectNode finalReview(String timelineId,ObjectNode body){return store.transaction(()->{ObjectNode timeline=store.getForUpdate(TIMELINE,timelineId);if(text(timeline,"finalUrl").isBlank()||!timeline.path("finalTechnicalQa").path("passed").asBoolean())throw new WorkflowException("FINAL_TECHNICAL_QA_REQUIRED","请先完成通过技术质检的最终渲染");String decision=body.path("decision").asText().toUpperCase(Locale.ROOT);if(!Set.of("PASS","REGENERATE","MANUAL_FIX").contains(decision))throw new IllegalArgumentException("终片创作验收只支持 PASS、REGENERATE 或 MANUAL_FIX");ObjectNode review=obj().put("decision",decision).put("reviewer",body.path("reviewer").asText("LOCAL_OPERATOR")).put("notes",body.path("notes").asText("")).put("reviewedAt",java.time.Instant.now().toString());for(String field:List.of("characterConsistency","propContinuity","positionContinuity","actionContinuity","editingRhythm","dialogueQuality","bgmFit","sfxAccuracy","subtitleAccuracy","hook","midHook","cliffhanger")){int score=body.path(field).asInt(0);if(score<0||score>5||"PASS".equals(decision)&&score==0)throw new IllegalArgumentException(field+" 通过时必须评为 1～5 分");review.put(field,score);}ObjectNode next=timeline.deepCopy().put("finalQaStatus","PASS".equals(decision)?"PASSED":decision);next.set("finalCreativeQa",review);return store.update(TIMELINE,timelineId,revision(timeline),next);});}
