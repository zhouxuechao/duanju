...
public class ProductionInputSnapshotService {
    ...
    private String scriptId(String sourceId,JsonNode input){String direct=first(text(input,"scriptVersionId"),text(input.path("context").path("episodeScript"),"scriptVersionId"),text(input.path("episode"),"scriptVersionId"));if(!direct.isBlank())return direct;String episodeId=first(text(input,"episodeId"),text(input.path("episode"),"id"));JsonNode shot=shot(sourceId,input);if(episodeId.isBlank()&&shot.isObject())try{episodeId=required(store.get(SCENE,required(shot,"sceneId")),"episodeId");}catch(RuntimeException ignored){}if(!episodeId.isBlank())try{ObjectNode episode=store.get(EPISODE,episodeId);return first(text(episode,"confirmedScriptVersionId"),text(episode,"scriptVersionId"),text(episode,"storyDocumentId"));}catch(RuntimeException ignored){}return first(text(input.path("context").path("episodeScript"),"storyDocumentId"),text(input.path("episode"),"storyDocumentId"));}
