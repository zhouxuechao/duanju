...
public class WorkflowService {
    ...
    public ObjectNode context(ObjectNode source){
        ...
        if(source.path("storyTime").isNumber()){
            ...
            ObjectNode factContext=factResolver.resolve(project(source),source.path("storyTime").asDouble(),visible);context.set("storyFacts",factContext.path("facts").deepCopy());context.set("characterKnowledge",factContext.path("characterKnowledge").deepCopy());
            ...
            ObjectNode propStates=obj();source.path("propIds").forEach(value->{String propId=value.asText();ObjectNode resolved=propStateResolver.resolve(project(source),propId,source.path("storyTime").asDouble());propStates.set(propId,resolved);ObjectNode state=(ObjectNode)shot.withObject("startState").withObject("props").withObject(propId);if(resolved.hasNonNull("heldByHand"))state.set("heldByHand",resolved.path("heldByHand").deepCopy());if(resolved.hasNonNull("carriedBy")){state.set("carriedBy",resolved.path("carriedBy").deepCopy());state.set("holder",resolved.path("carriedBy").deepCopy());}});context.set("propStates",propStates);
    ...
    private ObjectNode assets(String projectId,AssetDependencyAnalyzer.Level level){ObjectNode assets=obj();String coreId=required(store.get(PROJECT,projectId),"activeStoryDocumentId");for(ResourceKind kind:List.of(CHARACTER,CHARACTER_LOOK,LOCATION,PROP)){ArrayNode array=assets.putArray(kind==CHARACTER_LOOK?"looks":kind.path());List<ObjectNode> ordered=new ArrayList<>(store.list(kind,projectId,null));ordered.sort(Comparator.comparingInt((ObjectNode value)->value.path("storyOrder").asInt(Integer.MAX_VALUE)).thenComparing(value->assetKey(value,kind)).thenComparing(value->id(value)));for(ObjectNode a:ordered)if(coreId.equals(text(a,"storyBibleId"))&&!a.path("stale").asBoolean()){ObjectNode value=a.deepCopy();if(kind!=CHARACTER){ArrayNode views=value.putArray("approvedViews");assetViews.approvedReferences(projectId,coreId,id(a),level).forEach(views::add);}array.add(value);}}return assets;}
    private String assetKey(JsonNode value,ResourceKind kind){return switch(kind){case CHARACTER->text(value,"characterKey");case CHARACTER_LOOK->text(value,"lookKey");case LOCATION->text(value,"locationKey");case PROP->text(value,"propKey");default->"";};}
    ...
    private ObjectNode savePrompt(ObjectNode shot,String purpose,JsonNode compiled,JsonNode context){
        ...
        ObjectNode prompt=obj().put("id",UUID.randomUUID().toString()).put("projectId",project(shot)).put("shotId",id(shot)).put("purpose",purpose).put("version",version)
        ...
        copyPromptAudit(prompt,compiled,context);prompt.set("inputSnapshot",context.deepCopy());ObjectNode input=obj().put("promptVersionId",id(prompt)).put("continuitySnapshotHash",text(prompt,"continuitySnapshotHash"));input.set("context",context.deepCopy());input.set("assetReferences",compiled.path("references").deepCopy());copyProvenance(prompt,productionSnapshots.describe(project(shot),id(shot),"PROMPT",input));return store.create(PROMPT_VERSION,prompt);
    }
    ...
    private void copyProvenance(ObjectNode target,JsonNode source){for(String field:List.of("assetSnapshotHash","scriptVersionId","scriptHash","characterDefinitionVersionIds","characterVersionIds","characterLookVersionIds","locationDefinitionVersionIds","locationVersionIds","propDefinitionVersionIds","propVersionIds","storyFactVersions","storyFactHash","knowledgeSnapshotHash","relationshipSnapshotHash","continuitySnapshotHash","unknownFields"))if(source.has(field))target.set(field,source.path(field).deepCopy());}
