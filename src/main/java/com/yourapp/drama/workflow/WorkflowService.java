...
public class WorkflowService {
    ...
    public ObjectNode image(String shotId,String type,ObjectNode body){
        ...
        return store.transaction(()->{
            ...
            ObjectNode context=context(shot);context.put("providerMediaType","IMAGE").put("imageTaskType",mediaPurpose.name()).put("mediaPurpose",mediaPurpose.name());context.set("providerCapabilities",production.imageCapabilities());context.set("outputProfile",outputProfile(project(shot)));validateCharacters(context.path("assets"),shot);
            ...
            imageOptions.put("size",switch(ratio){case "9:16"->"1440x2560";case "16:9"->"2560x1440";case "1:1"->"2048x2048";default->throw new WorkflowException("UNSUPPORTED_RATIO","项目画幅必须为 9:16、16:9 或 1:1");});
    ...
    private VideoPreparation prepareVideoRequest(ObjectNode keyframe,ObjectNode shot,ObjectNode body){
        ...
        ObjectNode context=context(shot);context.put("providerMediaType","VIDEO");context.set("keyframe",keyframeSnapshot.deepCopy());context.set("providerCapabilities",production.videoCapabilities());context.set("videoOutputProfile",outputProfile(project(shot)));attachRetake(context,shot);
        ...
        ObjectNode planningBody=body.deepCopy(),options=obj().put("duration",(int)Math.ceil(shot.path("duration").asDouble(3))).put("ratio",store.get(PROJECT,project(shot)).path("ratio").asText("9:16"));
    ...
    public ObjectNode videoPreview(String keyframeId,ObjectNode body){
        return store.transaction(()->{
            ...
            for(String field:List.of("modelId","modelProfileVersion","capabilityFingerprint","taskType","lockMode","route","videoRequestRoute","activatedMaterials","excludedMaterials","referenceMapping","referenceAuthority","referenceBudget","providerParameters","rulePackFingerprint","rulePackUpstreamCommit","runtimeRuleIds","audioGenerationPolicy","modelProfile","preflight","references","firstFrameProviderUrl"))if(prepared.requestPlan().has(field))preview.set(field,prepared.requestPlan().path(field).deepCopy());
    ...
    public ObjectNode review(ResourceKind kind,String id,ObjectNode body){
        ...
        return store.transaction(()->{
            ...
            ObjectNode qc=obj().put("projectId",project(item)).put("targetKind",kind.path()).put("targetId",id).put("shotId",required(item,"shotId"))
                .put("reviewSequence",nextReviewSequence(project(item),id)).put("passed",body.path("passed").asBoolean()).put("score",score).put("reviewer",reviewer).put("notes",body.path("notes").asText("人工审核"));
