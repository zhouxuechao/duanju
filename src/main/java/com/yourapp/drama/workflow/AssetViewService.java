...
public class AssetViewService {
    ...
    public AssetViewService(DocumentStore store,JobService jobs,ImageGenerator images,MediaStorage storage,ProviderMediaFetcher fetcher,PromptCompiler prompts){this.store=store;this.jobs=jobs;this.images=images;this.storage=storage;this.fetcher=fetcher;this.prompts=prompts;}
    ...
    private ObjectNode schedule(ObjectNode view){
        ...
        ArrayNode bindings=JsonNodeFactory.instance.arrayNode();for(JsonNode referenceId:references)bindings.add(referenceBinding(current,store.get(ASSET_VIEW,referenceId.asText())));
        String bindingHash=hash(bindings);ObjectNode input=obj().put("assetViewId",id(current)).put("coreId",text(current,"coreId")).put("assetKind",text(current,"assetKind")).put("assetId",text(current,"assetId")).put("view",text(current,"view")).put("sourceHash",text(current,"sourceHash")).put("imageTaskType","ASSET_REFERENCE").put("compilerVersion",compilerVersion(current)).put("referenceBindingsHash",bindingHash).put("referenceAuthorityFingerprint",bindingHash).put("continuitySnapshotHash",text(current,"sourceHash")).put("providerCapabilitiesVersion",ProviderCapabilityRegistry.VERSION);input.set("sourceSnapshot",current.path("sourceSnapshot").deepCopy());input.set("referenceViewIds",references);input.set("referenceBindings",bindings);
        ...
        var compiled=prompts.compileAssetReference(input);String compiledPrompt=compiled.prompt();input.put("compilerVersion",compiled.compilerVersion()).put("normalizedPromptHash",hash(TextNode.valueOf(compiledPrompt.replaceAll("\\s+"," ").trim()))).put("prompt",compiledPrompt);input.set("promptIR",compiled.promptIRJson().deepCopy());
    ...
    private AssetDependencyAnalyzer.Level dependencyLevel(ObjectNode project,JsonNode request){boolean phaseB=project.path("testRun").asBoolean()&&"PIPELINE".equalsIgnoreCase(text(project,"testPhase"))&&"phase-b-production".equals(text(project,"scenarioId"));if(phaseB){String stored=text(project,"assetDependencyLevel"),requested=text(request,"assetDependencyLevel");if(!"A1".equals(stored)||(!requested.isBlank()&&!"A1".equals(requested)))throw new WorkflowException("PIPELINE_EXECUTION_PROFILE_MISMATCH","Phase B Pipeline Canary 必须固定使用 A1 素材依赖");return AssetDependencyAnalyzer.Level.A1;}String configured=text(request,"assetDependencyLevel");if(!configured.isBlank())try{return AssetDependencyAnalyzer.Level.valueOf(configured);}catch(IllegalArgumentException error){throw new WorkflowException("ASSET_DEPENDENCY_INVALID","素材依赖等级无效："+configured);}String coreId=text(project,"activeStoryDocumentId");int subjects=0,locations=0;if(!coreId.isBlank()){subjects=(int)store.list(CHARACTER,project(project),null).stream().filter(a->coreId.equals(text(a,"storyBibleId"))&&!a.path("stale").asBoolean()).count();locations=(int)store.list(LOCATION,project(project),null).stream().filter(a->coreId.equals(text(a,"storyBibleId"))&&!a.path("stale").asBoolean()).count();}int episodes=Math.max(1,project.path("episodeCount").asInt(1));double seconds=Math.max(1,project.path("targetDuration").asDouble(24))*episodes;int shots=Math.max(1,(int)Math.ceil(seconds/3));return new AssetDependencyAnalyzer().classify(subjects,locations,shots,shots>1,episodes>=30);}
