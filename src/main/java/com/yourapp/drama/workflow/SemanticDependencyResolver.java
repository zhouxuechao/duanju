...
public class SemanticDependencyResolver {
    public ObjectNode resolve(String sourceKind,String sourceId,JsonNode before,JsonNode after){Set<String> changed=new TreeSet<>();Set<String> names=new TreeSet<>();before.fieldNames().forEachRemaining(names::add);after.fieldNames().forEachRemaining(names::add);for(String field:names)if(!Objects.equals(before.get(field),after.get(field))&&!Set.of("revision","updatedAt","impactDecision").contains(field))changed.add(field);String kind=sourceKind.toLowerCase(Locale.ROOT),action="NO_IMPACT",explain="仅编辑展示信息，无须重做已生成内容";ArrayNode impacts=JsonNodeFactory.instance.arrayNode();
        ...
        else if(kind.equals("dialogue-lines")&&changed.contains("semanticText")){action="REGENERATE";explain="剧情对白语义变化，需要显式重建 TTS 发音和字幕轨并使时间线失效";impact(impacts,"TTS","REGENERATE");impact(impacts,"SUBTITLE","REGENERATE");impact(impacts,"TIMELINE","STALE");}
        else if(kind.equals("dialogue-lines")&&changed.stream().anyMatch(Set.of("spokenText","voiceProfileId","dialect")::contains)){action="REGENERATE";explain="发音文本、方言或音色变化，只重做 TTS、LIPSYNC 并使时间线失效";impact(impacts,"TTS","REGENERATE");impact(impacts,"LIPSYNC","REGENERATE");impact(impacts,"TIMELINE","STALE");}
        else if(kind.equals("dialogue-lines")&&changed.contains("subtitleText")){action="REVALIDATE";explain="字幕文本变化，只重做字幕并重新校验时间线";impact(impacts,"SUBTITLE","REGENERATE");impact(impacts,"TIMELINE","REVALIDATE");}
