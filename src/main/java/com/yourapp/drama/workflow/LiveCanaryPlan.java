...
public record LiveCanaryPlan(String phase,int imageRequests,int videoRequests,int audioRequests,int llmRequests,int vlmRequests,
    ...
    public ObjectNode validateLive(Map<String,String> environment,List<? extends JsonNode> jobs){
        ...
        if("PIPELINE".equals(phase)){
            check(checks,blocking,"PIPELINE_LIVE_FLAG","true".equalsIgnoreCase(environment.get("RUN_LIVE_PIPELINE_CANARY")),"RUN_LIVE_PIPELINE_CANARY 必须显式为 true");
            check(checks,blocking,"PIPELINE_RUNNER_PREFLIGHT","true".equalsIgnoreCase(environment.get("CANARY_PIPELINE_RUNNER_PREFLIGHT_OK")),"真实 Pipeline Canary 必须由 run-pipeline-canary.ps1 完成安全预检后启动");
        }else{
    ...
    public void requireLiveFlags(Map<String,String> environment){
        if("PIPELINE".equals(phase)){
            if(!"true".equalsIgnoreCase(environment.get("RUN_LIVE_PIPELINE_CANARY")))throw new IllegalStateException("RUN_LIVE_PIPELINE_CANARY=true 才能启动完整 Pipeline Canary");
            if(!"true".equalsIgnoreCase(environment.get("CANARY_PIPELINE_RUNNER_PREFLIGHT_OK")))throw new IllegalStateException("CANARY_PIPELINE_RUNNER_PREFLIGHT_REQUIRED: 真实 Pipeline Canary 必须由 run-pipeline-canary.ps1 完成安全预检后启动");
        }else{
