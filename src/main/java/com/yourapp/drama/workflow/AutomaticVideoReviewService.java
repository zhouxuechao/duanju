...
public class AutomaticVideoReviewService {
    ...
    public ObjectNode review(String takeId,JsonNode request){
        String assessmentKey=request.path("requestKey").asText("vlm:video:v"+VisualQualityProtocol.VERSION+":"+takeId+(request.path("apply").asBoolean(false)?":apply":":shadow"));synchronized(LOCKS[Math.floorMod(assessmentKey.hashCode(),LOCKS.length)]){return reviewOnce(takeId,request,assessmentKey);}
    }
    private ObjectNode reviewOnce(String takeId,JsonNode request,String assessmentKey){
        ...
        JsonNode result;try{result=protocol.validate(reviewer.review(expected,extractor.extract(media.requiredArchiveKey(take))),expected);}catch(ProviderException failure){failedAssessment(take,assessmentKey,shadow,failure);throw failure;}
