...
public class PipelinePreflightService {
    ...
    public ObjectNode review(String projectId,String requestedMode){String mode=requestedMode==null||requestedMode.isBlank()?providerMode.toUpperCase(Locale.ROOT):requestedMode.toUpperCase(Locale.ROOT);if(!Set.of("MOCK","MEDIA","CANARY","LIVE").contains(mode))throw new IllegalArgumentException("自检模式只支持 MOCK、MEDIA、CANARY 或 LIVE");ArrayNode checks=JsonNodeFactory.instance.arrayNode(),blocking=JsonNodeFactory.instance.arrayNode(),warnings=JsonNodeFactory.instance.arrayNode();ObjectNode project;
        ...
        if(Files.isRegularFile(Path.of("skills/00-premise-analysis/prompt.md"))&&Files.isRegularFile(Path.of("skills/01-story-planning/prompt.md"))&&Files.isRegularFile(Path.of("skills/04-script-writing/prompt.md")))pass(checks,"STORY_SKILLS_OK","故事阶段规则可用");else fail(checks,blocking,"STORY_SKILLS_MISSING","故事阶段规则文件不完整",false,"恢复 skills 中的前提分析、故事规划和剧本规则");
        if(Files.isRegularFile(Path.of("skills/vendor/short-drama-factory/UPSTREAM.md")))pass(checks,"VENDOR_RULES_OK","上游编剧规则版本可追溯");else fail(checks,blocking,"VENDOR_RULES_MISSING","上游编剧规则缺少版本记录",false,"恢复 vendor 规则与 UPSTREAM.md");
