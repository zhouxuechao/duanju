    ...
    ObjectNode run(Path root){
        ...
        try(var folders=Files.list(fixtures)){
            for(Path folder:folders.filter(Files::isDirectory).sorted().toList()){
                ...
                String forbidden=fixture.path("expectedInvariants").path("coreLoopMustNotContain").asText();if(!forbidden.isBlank()&&pack.path("storyTypeRule").path("coreLoop").asText().contains(forbidden))issues.add("STORY_TYPE_RULE_CONTAMINATION:"+forbidden);
                String requiredBeatMode=fixture.path("expectedInvariants").path("requiredBeatMode").asText();if(!requiredBeatMode.isBlank()&&!requiredBeatMode.equals(format.path("beatMode").asText()))issues.add("BEAT_MODE_REGRESSION:"+requiredBeatMode);
                int maxSceneLimit=fixture.path("expectedInvariants").path("maxSceneLimit").asInt(0);if(maxSceneLimit>0&&format.path("sceneLimit").asInt()>maxSceneLimit)issues.add("SCENE_LIMIT_REGRESSION:"+maxSceneLimit);
