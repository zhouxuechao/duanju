...
blockingIssues 只放必须修复的问题，例如未来事实泄漏、关键连续性错误、无叙事增量、LONG 缺有效中段钩子、Round B 重复 Round A、核心兑现被中段提前耗尽、类型发动机错用。issues 放非阻断问题。rewriteInstructions 必须具体说明修改哪个局部、保留哪些已确认边界。blockingIssues 非空时 passed 必须为 false，rewriteRequired 必须为 true。只返回 Schema 对应 JSON。
